...
class CFG {
    ...
    public:
        ...
        T get(const char* path) {
            T invalid{0};
            if (settings.find(path) != settings.end()) {
                const auto& entry = settings[path];
                return std::visit([&path](const auto& e) -> T {
                    T invalid{0};
                    ...
                    std::cout << "[ERROR:config.hpp] Invalid configuration get request for " << path << std::endl;
                    return invalid;
                }, entry);
            } else {
                std::cout << "[ERROR:config.hpp] Invalid path " << path << std::endl; 
            }
            return invalid;
        };
        
        template<typename T>
        int set(const char* path,const T v) {
            if (settings.find(path) != settings.end()) {
                auto& entry = settings[path];
                    ...
                    if constexpr (std::is_same_v<U, bolEntry>) {
                        if constexpr (std::is_same_v<T, bool>) {
                            if (e.isValid(v)) {
                                ...
                                std::cout << "[ERROR:config.hpp] Invalid value " << path << "=" << v << std::endl;   
                            }  
                        }            
                    } else if constexpr (std::is_same_v<U, intEntry>) {
                        if constexpr (std::is_same_v<T, int>) {
