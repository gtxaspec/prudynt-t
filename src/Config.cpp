...
Config::Config() {
    ...
    std::vector<ConfigItem<int>> intItems = {
        ...
        {"sensor.fps", sensorFps, 24, [](const int &v) { return v > 0 && v <= 60; }, "Sensor FPS must be between 1 and 60"},
        ...
        {"stream0.osd_pos_x", stream0osdPosX, 5, [](const int &v) { return v >= 0; }, "X position for OSD in stream 0"},
        {"stream0.osd_pos_y", stream0osdPosY, 5, [](const int &v) { return v >= 0; }, "Y position for OSD in stream 0"},
        ...
        {"osd.font_stroke_size", OSDFontStrokeSize, 2, [](const int &v) { return v >= 0; }, "OSD font stroke size should be non-negative"},
