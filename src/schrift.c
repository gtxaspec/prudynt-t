...
static void *rreallocarray(void *optr, size_t nmemb, size_t size);
...
static void *
rreallocarray(void *optr, size_t nmemb, size_t size)
{
...
static void offset_outline(Outline *outl, double offset) {
    for (unsigned int i = 0; i < outl->numPoints; ++i) {
        Point *p = &outl->points[i];
        double length = sqrt(p->x * p->x + p->y * p->y);
        p->x += offset * p->x / length;
        p->y += offset * p->y / length;
    }
}

int sft_render_with_outline(const SFT *sft, SFT_Glyph glyph, SFT_Image image, double outlineOffset) {
    uint_fast32_t outline;
    double transform[6];
    int bbox[4];
    Outline outl;
    ...
    offset_outline(&outl, outlineOffset);
    if (render_outline(&outl, transform, image) < 0)
        goto failure;
